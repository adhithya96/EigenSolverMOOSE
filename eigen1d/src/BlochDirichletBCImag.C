...
#include "BlochDirichletBCImag.h"

registerMooseObject("MooseApp", BlochDirichletBCImag);

InputParameters
BlochDirichletBCImag::validParams()
{
...
BlochDirichletBCImag::BlochDirichletBCImag(const InputParameters & parameters)
...
ADReal
BlochDirichletBCImag::computeQpValue()
{
  //Get point  locator  
  ...
  return _coef(space_arg, time_arg) * (_ur(space_arg, time_arg) * sin (_wave_num * _lattice_vec)  -  _uim(space_arg, time_arg) * cos (_wave_num * _lattice_vec));
