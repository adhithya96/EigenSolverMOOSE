...
Eigen::VectorXd Quad4::get_coordinates(int nodenum)
{
...
Eigen::VectorXd Quad4::get_connectivity(int elenum)
{
...
Quad4::Quad4(const int nelex, const int neley, const double lx, const double ly)
{
    const double dx = lx / (4 * nelex);
    const double dy = ly / (4 * neley);
    int nnodex = 4 * nelex + 1;
    int nnodey = 4 * neley + 1;
    nnode = nnodex * nnodey;
    nelem = nelex * neley;
    nen = 25;
    ndof = 4;
    ...
    for(int j = 0; j < neley; j++)
    {
        ...
        {
            elem(i + j * nelex, 0) = 4 * j * nnodex + 4 * i;
            elem(i + j * nelex, 1) = 4 * j * nnodex + 4 * i + 4;
            elem(i + j * nelex, 2) = 4 * (j + 1) * nnodex + 4 * i + 4;   
            elem(i + j * nelex, 3) = 4 * (j + 1) * nnodex + 4 * i;
            elem(i + j * nelex, 4) = 4 * j * nnodex + 4 * i + 1;
            elem(i + j * nelex, 5) = 4 * j * nnodex + 4 * i + 2;
            elem(i + j * nelex, 6) = 4 * j * nnodex + 4 * i + 3;
            elem(i + j * nelex, 7) = (4 * j + 1) * nnodex + 4 * i + 4;
            elem(i + j * nelex, 8) = (4 * j + 2) * nnodex + 4 * i + 4;
            elem(i + j * nelex, 9) = (4 * j + 3) * nnodex + 4 * i + 4;
            elem(i + j * nelex, 10) = 4 * (j + 1) * nnodex + 4 * i + 3;
            elem(i + j * nelex, 11) = 4 * (j + 1) * nnodex + 4 * i + 2;
            elem(i + j * nelex, 12) = 4 * (j + 1) * nnodex + 4 * i + 1;
            elem(i + j * nelex, 13) = (4 * j + 3) * nnodex + 4 * i;
            elem(i + j * nelex, 14) = (4 * j + 2) * nnodex + 4 * i;
            elem(i + j * nelex, 15) = (4 * j + 1) * nnodex + 4 * i;
            elem(i + j * nelex, 16) = (4 * j + 1) * nnodex + 4 * i + 1;
            elem(i + j * nelex, 17) = (4 * j + 1) * nnodex + 4 * i + 2;
            elem(i + j * nelex, 18) = (4 * j + 1) * nnodex + 4 * i + 3;
            elem(i + j * nelex, 19) = (4 * j + 2) * nnodex + 4 * i + 3;
            elem(i + j * nelex, 20) = (4 * j + 3) * nnodex + 4 * i + 3;
            elem(i + j * nelex, 21) = (4 * j + 3) * nnodex + 4 * i + 2;
            elem(i + j * nelex, 22) = (4 * j + 3) * nnodex + 4 * i + 1;
            elem(i + j * nelex, 23) = (4 * j + 2) * nnodex + 4 * i + 1;
            elem(i + j * nelex, 24) = (4 * j + 2) * nnodex + 4 * i + 2;
        }
    }
}

Eigen::VectorXd Quad4::RefElementCoord(int nodenum)
{
    Eigen::VectorXd x(2);
    switch(nodenum)
    {
...
Eigen::VectorXd Quad4::GaussIntegrationWeights(int nodenum)
{
    Eigen::VectorXd w(2);
    switch(nodenum)
    {
...
double Quad4::get_N(int i, double exi, double eta)
{
    //Get the reference  coordinates
    Eigen::VectorXd x = RefElementCoord(i);


    Eigen::VectorXd  refx(5);
    refx << -1,-0.6546,0,0.6546,1;
    Eigen::VectorXd refy(5); 
    refy << -1,-0.6546,0,0.6546,1;
...
double Quad4::get_dNdx(int i, int j, double exi, double eta)
{
    Eigen::VectorXd x = RefElementCoord(i);
...
//    std::cout << x << std::endl;
    //std::cout << x << std::endl;
    Eigen::VectorXd exik(5);
    exik << -1,-0.6546,0,0.6546,1;
    Eigen::VectorXd etal(5);
    etal << -1,-0.6546,0,0.6546,1;
...
Eigen::MatrixXd Quad4::get_jacobianmat(Eigen::MatrixXd x, double exi, double eta)
{
...
Eigen::MatrixXd Quad4::get_localstiffness(Eigen::MatrixXd D, Eigen::MatrixXd node)
{
    //gaussian points
    Eigen::VectorXd x(2);
...
Eigen::MatrixXd Quad4::get_localmass(double rho, Eigen::MatrixXd node)
{
    //std::cout << x << std::endl;
...
Eigen::MatrixXd Quad4::get_constraintmatrix(double kx, double ky, double a, int nnodex, int nnodey)
{
